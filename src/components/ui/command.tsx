...
>(({ className, ...props }, ref) => (
	<CommandPrimitive
		ref={ref}
		className={cn(
			"text-whit flex h-full w-full flex-col overflow-hidden rounded-md bg-transparent focus-visible:outline-none",
			className,
		)}
		{...props}
	/>
...
const CommandDialog = ({
	...
	...props
}: CommandDialogProps & { loop?: boolean; disablePointerSelection?: boolean }) => {
	return (
		<Dialog {...props}>
			<DialogContent className="overflow-hidden border-none bg-[rgba(0,0,0,0.3)] p-0 backdrop-blur-xl">
...
>(({ className, ...props }, ref) => (
	<div className="flex items-center border-b bg-transparent text-white" cmdk-input-wrapper="">
		<MagnifyingGlassIcon className="ml-2 mr-2 h-4 w-4 opacity-50" />
