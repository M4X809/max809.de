...
export default async function ManageQrCodes({ id }: { id: User["id"] }) {
			...
			<Box className="grid grid-cols-1 gap-4 md:grid-cols-2">
				...
				<Box>
					{(await hasPermission("changeQrLimits")) && (
						<SetLimitInput id={id} session={session} currentLimit={qrData.limit} />
					)}
					{!(await hasPermission("changeQrLimits")) && (
						<Center className="m-auto h-full rounded-lg bg-[rgba(255,255,255,0.1)] p-2">
				...
				<Box className="h-full rounded-lg bg-[rgba(255,255,255,0.1)] p-2">
					{qrData.codes.length > 0 && (
						<ScrollArea
							scrollbars="y"
							className="h-[calc(3rem*4+6px)] w-full md:h-[calc(3rem*3+4px)]"
							type="auto"
							offsetScrollbars
						>
							<Stack gap={2}>
								{qrData.codes.map(async (code, index) => {
									return (
										<Box key={code.id} className="grid h-12 grid-cols-6 gap-4">
											...
											{code.shareable && code.qrCode && (
												<Text
													component="span"
													title="Public Code"
													className="col-span-1 flex place-items-center align-middle"
													fz={18}
													px={3}
													c={"lightgray"}
												>
											...
											{(await hasPermission(["viewQrPreview", "deleteQrCode"])) && (
												<Box className="col-span-2 flex place-items-center justify-end px-5">
													<ActionGroup session={session} qrId={code.id} userId={id} />
												</Box>
											)}
										</Box>
									);
					...
					{qrData.codes.length === 0 && (
						<Box className="flex h-full place-items-center justify-center gap-4">
							<Stack>
								<Text fz={18} fw={500}>
									No QR Codes Found
								</Text>
	...
	return (
		<AccordionItem value="qr-codes" className="rounded-lg bg-[rgba(255,255,255,0.1)] backdrop-blur-lg">
			<AccordionControl>
