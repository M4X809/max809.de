...
export const PreviewCode = () => {
	...
	const { data: previewCode, isLoading: isLoadingPreviewCode } = api.management.getPreviewQr.useQuery(
		{ qrId: previewCodeId ?? "" },
		{ enabled: !!previewCodeId },
	);
	...
	return (
		...
		>
			{(!previewCode || isLoadingPreviewCode) && (
				<LoadingOverlay
					transitionProps={{
						duration: 500,
					}}
			...
			{!!previewCode && (
				<>
					<LoadQrConfig autoLoad data={previewCode} />
					<Center>
						<MyCanvas />
					</Center>
				</>
...
export const SetLimitInput = ({
	currentLimit,
}: {
	id: string;
	session: SessionType;
	currentLimit: string | undefined | number;
}) => {
		...
		<TextInput
			...
			onBlur={() => {
				const newLimit = z
					...
					.min(1)
					.max(10000)
					.safeParse(typeof limit === "string" ? Number.parseInt(limit) : limit);
				if (newLimit.error) {
