...
export default async function UserPage({
	...
	const accountActions = async () => {
		if (
			await hasPermission(["deleteUser", "resetPermissions", "logoutAllDevices"])
		)
			return (
				<React.Fragment>
					<Title order={4}>Account Actions</Title>

					<Box className="grid gap-4 sm:grid-cols-4 md:grid-cols-3">
						{(await hasPermission("deleteUser")) && (
							<Group className="col-span-2 flex md:col-span-1">
						...
						<Card px={25} className={twMerge(className)}>
							...
							<Stack justify="center" gap={1}>
								...
								<Center ta={"center"} className="min-h-[1rem]">
									{user.admin && (
										<ClientIcon title="Administrator" icon={faUserShield} />
									)}
									{!user.admin && user.staff && (
										<ClientIcon title="Staff Member" icon={faUser} />
									)}
								</Center>
							</Stack>
						</Card>
	...
	return (
		<Container fluid size={"xl"} px={{ base: 0, md: undefined }}>
			<Grid pb={50} columns={10}>
				<GridCol span={{ base: 10, md: 2.5 }}>{userBody()}</GridCol>
				<GridCol span={{ base: 10, md: 7.5 }}>{MainBody()}</GridCol>
			</Grid>
		</Container>
	);
}
