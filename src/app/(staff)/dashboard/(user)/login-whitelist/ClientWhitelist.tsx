...
export const ActionButtons = ({
	...
	const {
		mutate: setAllowed,
		isPending: isPendingAllowed,
		isSuccess: isSuccessAllowed,
		error: errorAllowed,
	} = api.whitelist.changeStatus.useMutation();
	...
	useEffect(() => {
		if (errorAllowed) {
			...
			toast.error("Error changing status.", {
				...
				description: <Box>{errMessage()}</Box>,
			});
		}
	}, [errorAllowed]);
	...
	return (
		<Group wrap="nowrap" gap={5}>
			{hasPermission("editWhitelistStatus") && (
				<ActionIcon
					size={"sm"}
			...
			{hasPermission("deleteWhitelistEntry") && (
				<ActionIcon size={"sm"} disabled className="bg-[rgba(0,0,0,0.15)] hover:bg-[rgba(0,0,0,0.2)]">
		...
		<Group justify="end" wrap="nowrap">
			...
			<Group py={10} wrap="nowrap" justify="space-between">
				...
				<Group justify="end" gap={5}>
					<ActionIcon disabled={disabled() || page === 1} onClick={() => first()}>
					...
					<ActionIcon disabled={disabled() || page === totalPages} onClick={() => last()}>
