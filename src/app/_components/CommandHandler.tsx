...
const CommandHandler = ({
	...
	useHotkeys(
		keys as string | readonly string[],
		() => {
			toggle();
		},
		{
	...
	return (
		<CommandDialog open={opened} onOpenChange={toggle} loop>
			<CommandInput
				className="bg-[rgba(255,255,255,0.1)] px-3 text-white hover:bg-[rgba(255,255,255,0.12)]"
			...
			<CommandList className="bg-transparent">
				...
				{commands?.map((command) => {
					...
					if (command.type === "group") {
						...
						return (
							<CommandGroup heading={command.heading} key={command.heading}>
								{command.commands.map((_command) => {
									...
									if (command.requireStaff && !isStaff()) return null;
									if (command.onlyOnPath && !command.onlyOnPath.every((path) => currentPath.includes(path))) return null;
									if (typeof command.prefetch !== "undefined" && typeof window !== "undefined" && mounted) {
										router.prefetch(command.prefetch); /** console.log("prefetching", command.prefetch) */
									}
									return (
										<CommandItem
											...
											disabled={
												typeof command.disabled === "function"
													? command.disabled({ val: command.key, saveDisabled })
													: command.disabled
											}
										>
											...
											{loading === command.key && (
												<FontAwesomeIcon
													...
													fontSize={20}
													spin
													style={{ "--fa-animation-duration": "1s", "--fa-animation-timing": "ease-in" }}
												/>
											)}
