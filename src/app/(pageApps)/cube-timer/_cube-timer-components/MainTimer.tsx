...
export const MainTimer = ({
	session,
	...props
}: Omit<CenterProps, "children"> & { session: Session | null | undefined }) => {
		...
		if (type === "hintText") {
			switch (true) {
				...
				default:
					return (
						<Text component="span">
							Hold space to start! <br /> Tap space to get a new scramble.
						</Text>
		...
		<Center className={twMerge("w-full flex-grow", props.className)}>
			<Stack>
				...
				<Text
					ta={"center"}
					className={twMerge(
						"duration-250 text-sm text-red-500 text-opacity-50 transition-opacity",
						!isPending && "animate-bounce opacity-0",
					)}
				>
					Saving
				</Text>
