...
const CubeScrambleBox = ({ ...props }: Omit<ContainerProps, "children">) => {
	...
	const options = scrambleTypes.map((item) => (
		<Combobox.Option
			value={item.value}
			key={item.value}
			className="mb-1 bg-[rgba(255,255,255,0.1)] text-white last:mb-0 hover:bg-[rgba(255,255,255,0.12)]"
		>
			<Text c={"gray"} fz={13}>
				{item.label}
			</Text>
	...
	return (
		<Container
			className={twMerge(
				"max-h-fit min-h-fit w-full rounded-xl bg-[rgba(255,255,255,0.1)] md:max-h-[50%] md:min-h-[50%]",
				props.className,
		...
		>
			...
			>
				<Combobox.Target>
					...
					>
						{scrambleTypes.find((item) => item.value === scrambleType)?.label || (
				...
				<Combobox.Dropdown
					mah={250}
					className="overflow-auto border-none bg-gradient-to-tr from-[#06080f] to-[#122b69] text-white"
				>
					<Combobox.Options>{options}</Combobox.Options>
				</Combobox.Dropdown>
			</Combobox>
			{!!scramble && (
				<Center maw={"100%"} className="flex flex-col md:h-[calc(100%-100px)]">
					<Title c={"white"} className="text-center font-mono">
						{scramble}
					</Title>
				</Center>
			)}
