...
const CubeTimerHistory = ({
	history,
	...props
}: Omit<ContainerProps, "children"> & { history: CubeHistory | undefined }) => {
	...
	const [historyState, setHistoryState] = useState<CubeHistory>(
		history ?? { history: [], totalPages: 1, page: 1, total: 1 },
	);
	const { data, refetch, isFetching, isRefetching } = api.cube.getCubeTimeHistory.useQuery(
		{ cubeSize: scrambleType, page: page },
		{
	...
	return (
		<Container
			className={twMerge(
				"flex h-full max-h-[calc(100dvh-100px)] w-full flex-col rounded-xl bg-[rgba(255,255,255,0.1)]",
				props.className,
		...
		>
			<Box className="flex-grow pt-1">
					...
					historyState.history.map((data) => {
						return (
							<Box
								...
								key={data.id}
								className={twMerge(
									"duration-250 static left-0 top-0 transition-opacity",
									isFetching || isRefetching ? "animate-pules-fast" : "",
								)}
							>
								...
								<Group justify="space-between" align="baseline">
									...
									<ActionIcon
										...
										className="bg-red-500/50 text-white hover:bg-red-500/75"
										size={"sm"}
				...
				{!historyState.history.length && (
					<Center
						className={twMerge(
							"duration-250 static left-0 top-0 h-full transition-opacity",
							isFetching || isRefetching ? "animate-pules-fast" : "",
						)}
					>
			...
			<Group py={10} wrap="nowrap" justify="space-between">
				...
				<Group justify="end" gap={5}>
					<ActionIcon disabled={disabled()} onClick={() => first()}>
					...
					<ActionIcon disabled={disabled()} onClick={() => last()}>
