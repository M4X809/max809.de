...
export default async function EmojiFavicon() {
	...
	return (
		<HydrateClient>
			<Shell title="Emoji Favicon API" redirect={"/"} session={session}>
				<Container fluid className="h-full md:m-auto md:w-[1000px]">
					<Center>
						<Stack justify="center" gap={2} ta={"center"} mb={15}>
							...
							<Img
								...
								imgType="mantine"
								src={`${getDomain(env.NEXTAUTH_URL)}/api/icon/${topEmojis[0]?.emoji ?? "👑"}`}
								alt="emoji favicon"
								ratio={1 / 1}
								width={50}
							/>
					...
					<Container className="md:max-w-1/2 flex flex-wrap items-center gap-2">
						{topEmojis.map((emoji) => {
							...
							return (
								<Pill
									...
									className={twMerge("w-fit min-w-fit bg-[rgba(255,255,255,0.1)] px-1 text-white", admin && " ")}
									classNames={{
											...
											<Box>
												<form action={deleteEmojiWithId}>
													<ActionIcon
														type="submit"
														unstyled
														className="mx-auto px-1 text-[#6d6c6cc2] opacity-100 transition-colors duration-500 hover:text-[#ff0000]"
													>
														<FontAwesomeIcon
															icon={faX}
															style={
