...
export const ManagementStoreContext = createContext<ManagementStoreApi | undefined>(undefined);

export interface ManagementStoreProviderProps {
	children: ReactNode;
}

export const ManagementStoreProvider = ({ children }: ManagementStoreProviderProps) => {
	const storeRef = useRef<ManagementStoreApi>(null);
	if (!storeRef.current) {
		storeRef.current = createManagementStore();
	}

	return <ManagementStoreContext.Provider value={storeRef.current}>{children}</ManagementStoreContext.Provider>;
};

export const useManagementStore = <T,>(selector: (store: ManagementStore) => T): T => {
